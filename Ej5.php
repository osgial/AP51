...
<body>
    ...
    function edadHumana(&$data){
        foreach ($data as $key => $value) {
            $edad=$value["edad"];
            $edadmod=31+16*log($edad);
            $data[$key]["edad"]=$edadmod;
        }
    }
    edadHumana($data);
    ...
    function sobrePeso($data){
        foreach ($data as $key => $value) {
            if ($value["edad"] >= 60) {
                if ($value["peso"] >= 7) {
                    echo "El perro " . $value["nombre"] . " con un peso de " . $value["peso"] . "y una edad de perro equivalente a " . $value["edad"] . " de edad humana<br>Tiene sobrepeso<br><br>";
                } else {
                    echo "El perro " . $value["nombre"] . " con un peso de " . $value["peso"] . "y una edad de perro equivalente a " . $value["edad"] . " de edad humana<br>No tiene sobrepeso<br><br>";
                }
            } elseif ($value["edad"] <= 40) {
                if ($value["peso"] >= 3) {
                    echo "El perro " . $value["nombre"] . " con un peso de " . $value["peso"] . "y una edad de perro equivalente a " . $value["edad"] . " de edad humana<br>Tiene sobrepeso<br><br>";
                } else {
                    echo "El perro " . $value["nombre"] . " con un peso de " . $value["peso"] . "y una edad de perro equivalente a " . $value["edad"] . " de edad humana<br>No tiene sobrepeso<br><br>";
